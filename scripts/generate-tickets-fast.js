...
/**
 * Fast Ticket Generation Script
 * Generates tickets and stores them in MongoDB only (no vectorization)
 * You can vectorize later in batches
 */
...
class FastTicketGenerator {
    ...
    async initialize() {
        try {
            console.log('🚀 Initializing Fast Ticket Generator...');
    ...
    /**
     * Save tickets to MongoDB only (no vectorization)
     */
    async saveTickets(tickets) {
        try {
            ...
            console.log('ℹ️ Vectorization skipped - you can run it later with: npm run vectorize:tickets');
    ...
    generateReport(tickets) {
        ...
        console.log('🚀 Next step: Run "npm run vectorize:tickets" to add vector embeddings');
    }
...
if (require.main === module) {
    const generator = new FastTicketGenerator();
    generator.run().then(() => {
        console.log('🎉 Fast ticket generation completed!');
        process.exit(0);
    }).catch(error => {
...
module.exports = FastTicketGenerator;
